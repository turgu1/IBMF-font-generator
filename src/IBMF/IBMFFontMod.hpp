...
public:
    ...
    inline auto getLineHeight(int faceIdx) const -> int {
        return ((faceIdx >= 0) && (faceIdx < preamble_.faceCount))
                   ? faces_[faceIdx]->header->lineHeight
                   : 0;
    }

    inline auto getFaceHeader(int faceIdx) const -> const FaceHeaderPtr {
        return ((faceIdx >= 0) && (faceIdx < preamble_.faceCount)) ? faces_[faceIdx]->header
                                                                   : nullptr;
    }
    ...
    auto save(std::ostream &out) -> bool;
    ...
    auto showBitmap(std::ostream &stream, const BitmapPtr bitmap) const -> void;
    auto showBackupLigKerns(std::ostream &stream, BackupGlyphLigKernPtr lk) const -> void;
    auto showLigKerns(std::ostream &stream, GlyphLigKernPtr lk) const -> void;
    auto showBackupGlyphInfo(std::ostream &stream, GlyphCode i, const BackupGlyphInfoPtr g) const
        -> void;
    auto showGlyphInfo(std::ostream &stream, GlyphCode i, const GlyphInfoPtr g) const -> void;
    auto showFace(std::ostream &stream, FacePtr face, bool withBitmaps) const -> void;
    auto showCodePointBundles(std::ostream &stream, int firstIdx, int count) const -> void;
    auto showPlanes(std::ostream &stream) const -> void;
    auto showFont(std::ostream &stream, std::string fontName, bool withBitmaps = false) const -> void;

    auto createBundleCodePointEntry(char16_t cPoint) -> void;
    void recomputeLigatures();
    auto importModificationsFrom(std::ostream &stream, std::string fontName, std::string fileName,
                                 IBMFFontModPtr fromBackup, IBMFFontModPtr toBackup,
                                 IBMFFontModPtr thisFont) -> void;
    auto buildModificationsFrom(std::ostream &stream, IBMFFontModPtr fromFont,
                                IBMFFontModPtr thisFont) -> IBMFFontModPtr;

    auto addCodePoint(IBMFFontModPtr backup, IBMFFontModPtr font, char32_t codePoint = 0)
        -> char32_t;

    auto glyphIsModified(int faceIdx, GlyphCode glyphCode, BitmapPtr &bitmap,
